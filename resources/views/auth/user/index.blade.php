...
<div class="">
    <div class="page-title">
      <div class="title_left">
        <h3> Data <small> pengguna</small> </h3>
      </div>
        ...
        <div class="x_panel">
          <div class="x_title">
            ...
            <ul class="nav navbar-right panel_toolbox">
              <li><a class="btn btn-success text-dark" href="{{ route('auth.user.create') }}">Buat Data Baru</a></li>
            </ul>
            <div class="clearfix"></div>
                      ...
                      <tbody>
                        @foreach ($users as $user)
                          <tr>
                            ...
                            <td>{{ $user->name }}</td>
                            <td>{{ $user->role }}</td>
                            <td>@if($user->gender  == 'LK') Laki - Laki @else Perempuan @endif</td>
                            <td>@if(is_null($user->company)) - @else {{ $user->company->name }} @endif</td>
                            <td>{{ $user->position_in_company }}</td>
                            <td>
                              <a target="_blank" class="btn btn-sm btn-secondary" href="{{ route('auth.user.show', ["id" => $user->id]) }}">Lihat</a>
                              <a target="_blank" class="btn btn-sm btn-primary" href="{{ route('auth.user.edit', ["id" => $user->id]) }}">Edit</a>
                            </td>
                          </tr>
                        @endforeach
                      </tbody>
